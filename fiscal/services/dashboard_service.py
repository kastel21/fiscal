...
def get_summary(device_id: int | None, range_key: str = "today") -> dict:
    """
    Aggregates FDMS-confirmed metrics only. Drafts never affect totals.
    """
    device = None
    if device_id:
        device = FiscalDevice.objects.filter(device_id=device_id, is_registered=True).first()
    if not device:
    ...
    submit_failures = FDMSApiLog.objects.filter(
    ...
    last_open = FDMSApiLog.objects.filter(endpoint__icontains="OpenDay").order_by("-created_at").first()
    last_close = FDMSApiLog.objects.filter(endpoint__icontains="CloseDay").order_by("-created_at").first()
...
def get_errors(device_id: int | None, range_key: str = "today") -> list:
    """Recent FDMS errors with operationID for linking and retry."""
    start_dt, end_dt = _date_range(range_key)
...
def get_receipts(device_id: int | None, range_key: str = "today", status_filter: str | None = None) -> list:
    """Receipt pipeline list for dashboard. Optional status_filter: draft|fiscalised|failed."""
    device = None
    if device_id:
        device = FiscalDevice.objects.filter(device_id=device_id, is_registered=True).first()
    if not device:
...
def get_quickbooks_stub() -> dict:
    """QuickBooks integration status for dashboard."""
    from fiscal.models import QuickBooksConnection, QuickBooksInvoice
    qs = QuickBooksInvoice.objects.all()
    ...
    return {
        ...
        "pending": pending,
        "failed": qs.filter(fiscalised=False).exclude(fiscal_error="").count(),
