...
def api_config_taxes(request):
    ...
    # Invoice dropdown: fixed taxes 0% (1, 2), 15.5% (517)
    INVOICE_TAXES = [
        {"taxID": 1, "taxCode": "1", "taxPercent": "0.00", "taxName": "0% (1)"},
        {"taxID": 2, "taxCode": "2", "taxPercent": "0.00", "taxName": "0% (2)"},
        {"taxID": 517, "taxCode": "517", "taxPercent": "15.50", "taxName": "15.5%"},
    ]
    if source_invoice:
        taxes = list(INVOICE_TAXES)
        if not is_vat_registered:
            taxes = [t for t in taxes if float(t.get("taxPercent") or 0) == 0]
        return JsonResponse({
            "taxes": taxes,
    ...
    if source_getconfig:
        ...
        for i, t in enumerate(tax_table):
            ...
            raw = t.get("taxCode")
            code = (str(raw).strip()[:TAX_CODE_MAX_LENGTH] if raw is not None else "") or str(t.get("taxName", "") or "").strip()[:TAX_CODE_MAX_LENGTH]
            if not code:
                continue
            ...
            taxes.append({
                "taxID": int(tid) if tid is not None else (i + 1),
                "taxCode": code,
                "taxPercent": format(round(float(pct), 2), ".2f"),
                "taxName": str(t.get("taxName", "") or code),
            })
        return JsonResponse({
            "taxes": taxes,
            "is_vat_registered": True,  # Don't filter for mapping form
            "configs_loaded": configs is not None,
        })
    ...
    if not taxes:
        ...
        if not taxes and configs:
            taxes = [{"taxID": 1, "taxCode": "VAT", "taxPercent": "15.00", "taxName": "VAT"}]

    if not is_vat_registered:
        taxes = [t for t in taxes if float(t.get("taxPercent") or 0) == 0]
        if not taxes and configs:
            taxes = [{"taxID": 1, "taxCode": "EXEMPT", "taxPercent": "0.00", "taxName": "Exempt (0%)"}]

    if not taxes and did is not None:
        taxes = [
            {"taxID": 1, "taxCode": "EXEMPT", "taxPercent": "0.00", "taxName": "Exempt (0%)"}
            if not is_vat_registered
            else {"taxID": 1, "taxCode": "VAT", "taxPercent": "15.00", "taxName": "VAT"}
        ]
    return JsonResponse({
        "taxes": taxes,
        "is_vat_registered": is_vat_registered,
...
def api_company(request):
    ...
    if not company:
        company = Company(
            ...
            currency_default=body.get("currency_default", "ZWL"),
        )
    else:
