...
class DeviceApiService(FDMSBaseService):
    ...
    def get_config(self, device: FiscalDevice) -> tuple[dict | None, str | None]:
        ...
        try:
            ...
            log_fdms_call(endpoint=endpoint, method="GET", request_payload={"deviceId": device_id}, error=str(e))
            return None, str(e)
    ...
    def issue_certificate(self, device: FiscalDevice) -> tuple[FiscalDevice | None, str | None]:
        ...
        try:
            ...
            log_fdms_call(endpoint=endpoint, method="POST", request_payload={"hasCsr": True}, error=str(e))
            return None, str(e)
        log_fdms_call(endpoint=endpoint, method="POST", request_payload={"hasCsr": True}, response=response)
        if response.status_code != 200:
            try:
                err_body = response.json()
                detail = err_body.get("detail", err_body.get("title", response.text))
