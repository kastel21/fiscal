...
def get_device_for_request(request):
    """
    Return the device for this request: from GET, POST, or session (system-wide selection).
    When device_id is in GET or POST, saves to session so selection persists.
    """
...
def dashboard(request):
    """Dashboard: device status, fiscal day info. Auto-refreshes. Device from GET or session."""
...
def receipt_history(request):
    """Receipt submission history page. Only filter by device when ?device_id= is set (default: show all)."""
...
def fiscal_day_dashboard(request):
    """Fiscal day state dashboard: devices and fiscal days overview."""
    devices = FiscalDevice.objects.filter(is_registered=True).order_by("device_id")
    fiscal_days = FiscalDay.objects.select_related("device").order_by("-opened_at")[:100]
    return render(
        request,
