...
def api_devices_list(request):
    """GET /api/devices/ - List registered devices for invoice form."""
...
def api_fdms_receipts(request):
    """GET /api/fdms/receipts/ - JSON list of receipts. Never allow deletion."""
    device_id = request.GET.get("device_id")
    queryset = Receipt.objects.select_related("device").order_by("-created_at")[:100]
...
def api_qb_validate_invoice_update(request):
    ...
    if receipt_id:
        ...
        receipt = Receipt.objects.filter(invoice_no=invoice_no).first()
